...
class Agent:
    def __init__(
        ...
        self.memory = Memory(
            ...
            assistant_name=self.identity,  # Directory to store memory files
            ...
            system_prompt=f"You are {self.identity}, {self.description}.",  # Provide Memory with agent's persona
        )
...
Emphasize the significance of adhering to the outlined procedures to ensure the seamless execution of tool chaining, error management, and the successful handling of user requests.
        ...
        if self.memory_enabled:
            ...
            response = self.llm.run(self.objective).strip()
        self.llm.reset()
    ...
    def rollout(self) -> str:
        ...
        self.llm.reset()
        if not self.tools:
            return self._run_no_tool() if self.objective else "No task provided."

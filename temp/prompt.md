...
class TaskForce:
    ...
    def rollout(self) -> str:
        ...
        json = self.llm.run("Generate JSON for the first task")
        self.llm.reset()
        json = self._parse_and_fix_json(json)
        print(f"{Fore.GREEN}{json}{Style.RESET_ALL}")
        agent= self._get_agent_by_name(json['agent_name'])
        task = json['objective']
    ...
    def _agent_pass(self, response) -> str:
        ...
        self.llm.__init__(
            ...
            **You are the Communication handler responsible for interconnecting agents.**
            ...
            You are tasked to pass necessary inforamtion to the next most suitable agent according to the objective.
            **Always Reply in JSON**
            ```json
            {{
                "agent": "<agent_name>",
                "prompt": "prompt to give to this agent for the task, write the prompt in markdown format and highlight the important task or information"
                "thought": "a summary of what did the previous agent do and whom you passed(if any) with the perspective of the previous agent"
            }}
            ```
            **If there is no need of any agent or any passing information, simply reply with an empty JSON object. - {{}}**

            ### INSTRUCTIONS:
            1. Pass the necessary information to the next most suitable agent.
            2. If there is no need of any agent or any passing information, simply reply with an empty JSON object. - {{}}
            3. Always prompt the agent in markdown format. And highlight the important task or information using `**`.
        ...
        if pass_info!={}:
            ...
            agent = pass_info["agent"]
            message = pass_info["prompt"]
            summary = pass_info["thought"]
            print("Response:")
            print(summary)
            agent = self._get_agent_by_name(agent)
            agent.objective = message
            agent.rollout()
        else:
            print(f"{Fore.BLUE}Final Consolidate response:{Style.RESET_ALL}")
            print(response)
            return response
...
Emphasize the significance of adhering to the outlined procedures to ensure the seamless execution of tool chaining, error management, and the successful handling of user requests. With adhering to different tool calls at once if needed.
    ...
    def _is_response_valid(self, response: str, call) -> bool:
        ...
        prompt = f"""
        You are an AI assistant tasked with evaluating the validity of a response. 

        Current Objective: {self.objective}
        Function Call: {call}
        Response: {response}

        Based on the current objective, is the response valid?
    ...
    def rollout(self) -> str:
        ...
        self.llm.reset()
        if self.tools!=[]:
            response = self._run_no_tool() if self.objective else "No task provided."
            self._agent_pass(response)
        else:
            self._agent_pass(self._run_with_tools())
```

**check everything and find flaws and logic error in code**
...
1. Current code of agent.py is automatic and the full process is managed by AI and the plan too add a method for manual use similar to crewai with my own process tool chaining.

2. You can add different classes for manual use like in crewAI in different file such as task.py which will be containing Task class (Just a suggestion you can implement your own class)

3. Introduce the concept of Iterations in the framework in both modes with max iterations - manual and AI

### Instructions:

**Always give full code without skipping anything.**